...
from utils.web import CaptchaError, Web, LoginError
...
def main(args) -> dict:
    ...
    pb_url_line = [x for x in web.response.text.split('\n') if '/get-member-passbook' in x][0]
    ...
    for account, mid in accounts.items():
        ...
        soup = BeautifulSoup(json.loads(web.response.text)[0]['html'], 'html.parser')
